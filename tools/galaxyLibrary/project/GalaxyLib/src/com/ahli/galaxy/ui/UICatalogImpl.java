...
public class UICatalogImpl implements UICatalog {
	...
	public Object deepCopy() {
		...
		final UICatalogImpl clone = new UICatalogImpl(templates.size() * 3 / 2 + 1, blizzOnlyTemplates.size(),
	...
	// * @throws DOMException
	// */
	// private void parseFrame(final Node node, final UIElement parent, final
	// String
	// fileName, final String raceId,
	// final boolean isDevLayout) throws UIException, DOMException,
	// InterruptedException {
	...
	// final UIElement templateElem = instanciateTemplate(template, name,
	// isDevLayout, parent);
	// final UIFrame thisElem = templateElem != null ? (UIFrame) templateElem :
	// new
	// UIFrame(name, type);
	//
	// // register with parent/templates
	...
	//
	// // 1. check templates
	// UIElement templateInstance = instanciateTemplateFromList(templates,
	// fileName,
	// path, newName);
	...
	// templateInstance = instanciateTemplateFromList(blizzOnlyTemplates,
	// fileName,
	// path, newName);
	// if (templateInstance != null) {
	// if (!isDevLayout) {
	...
	// if (!isDevLayout) {
	// logger.error("ERROR: Template of path '" + path + "' could not be
	// found.");
	// } else {
	// logger.warn("WARNING: Template of path '" + path
	// + "' could not be found, but we are creating a Blizz-only layout, so this
	// is
	// fine.");
	// }
	// return null;
